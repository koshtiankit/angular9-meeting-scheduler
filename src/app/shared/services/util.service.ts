...
export class UtilService {
  //use for first/last/middle name
  private _nameRegex: RegExp = /^[A-Za-z ,.'-]+$/i;
  public selectedLang: string;

  @Output() public languageChanged: EventEmitter<any> = new EventEmitter();
  ...
  checkCurrentLang() {
    ...
    console.log('this.selectedLang 38', this.selectedLang);

    this.selectLang(this.selectedLang);
  }

  public langSelected(event: any) {
    this.selectedLang = this.languages.filter(
      (val) => val.value === event
    )[0].value;
    localStorage.setItem('localeId', this.selectedLang);
    console.log('this.selectedLang util', this.selectedLang);
    this.selectLang(this.selectedLang);
  }
